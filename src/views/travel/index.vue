<template>
  <div class="page shopPage mrgb"
       id="travelBanner">
    <div v-for="(item, index) in shopData"
         :key="index">
      <flexbox :gutter="0"
               wrap="wrap"
               class='sortBox'
               v-if="item.layoutStyle == 'NavigationBtn'"
               id="length0topBar">
        <flexbox-item :span="0.33"
                      v-for='btn in item.imgList'
                      @click.native='init(btn.link)'
                      :key="btn.id">
          <div>
            <img :src="btn.imgUrl" />
      ...
      <swiper dots-position="center"
              ...
              v-if="item.layoutStyle=='Roll'"
              :show-dots="item.imgList.length>1"
              height="9.375rem">
      ...
      <div class="imgbox"
           v-if="item.layoutStyle=='TWO'">
        <div class="imgItem"
             v-for="imgList in item.imgList"
             @click='goLinkFn(imgList)'>
  ...
  methods: {
    init(link) {
      ...
      this.timer = setTimeout(() => {
        ...
        if (link) {
          tabId = link.split(":")[1]
          this.$http.get("h9/api/travel/health/" + tabId).then(res => {
            if (res.data.code == 0) {
              this.shopData = res.data.data
            }
          })
        } else {
          this.$http.get("h9/api/travel/health/").then(res => {
            if (res.data.code == 0) {
              this.shopData = res.data.data
            }
          })
