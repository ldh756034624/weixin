      ...
      <scroller lock-x
        ...
        <div class="mrgb">
          ...
          <flexbox :gutter="0"
                   wrap="wrap"
                   class='sortBox'>
            <flexbox-item :span="itembox"
                          v-for='item in navBanner'
                          @click.native='goLinkFn(item)'
                          :key="item.id">
              <div>
                <img :src="item.imgUrl" />
          ...
          <section>
            <div class="exchangeBox"
                 v-if='shopData.hotGoods'>
              <flexbox :gutter="0"
                       wrap="wrap"
                       align="flex-start">
                <flexbox-item :span="1/2"
                              class='shopLine'
                              v-for='item in shopData.hotGoods'
                              :key="item.id">
                  <router-link :to="{path:'/shopDataile',query:{id:item.id}}">
                    ...
                    <div class="bottomBox">
                      ...
                      <span class="joyMoney">
                        <span>¥ {{item.price}}</span>{{item.unit ? '/' + item.unit : ''}}</span>
                    </div>
                  </router-link>
                </flexbox-item>
              </flexbox>
...
export default {
  ...
  methods: {
    init() {
      this.$http.get("h9/store/home").then(res => {
        if (res.data.code == 0) {
    ...
    goLinkFn: function(item) {
      ...
      } else if (item.link.split(":")[0] === "bigRich") {
        // 跳转一号大富贵
        this.$router.push({ path: "/active/oneRich" })
      } else {
        //所有商品'allGoods',食品'foods',日常家居'everydayGoods',今日新品'todayNewGoods')
        this.$router.push({
          path: "/shopList",
          query: { type: item.link.split(":")[1], title: item.title }
        })
      }
    },
    refresh() {
...
.shopPage {
  ...
  .exchangeBox {
    ...
    .shopBox {
      ...
      box-shadow: 0px 4px 12px -1px rgba(77, 77, 77, 0.4);
