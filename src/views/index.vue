<template>
  <div class="page home"
      ...
      <scroller lock-x
        ...
        <div>
          <main class="home_main">
            ...
            <div class="flexBox verticalScroll"
                 v-if='homeData.noticeArticle && homeData.noticeArticle.length > 0'>
              <img src="../assets/img/index/mainpage_icon_notification_@2x.png"
                   class="indexRadio" />
              <div class="flex1">
                <swiper auto
                        height="40px"
                        ...
                        class=""
                        :show-dots="false">
                  <swiper-item v-for='item in homeData.noticeArticle'
                               @click.native='goLinkFn(item)'
                               :key="item.id">
            ...
            <flexbox :gutter="0"
              ...
              <flexbox-item :span="1/4"
                            v-for='item in homeData.navigationBanner'
                            @click.native='goLinkFn(item)'
                            :key="item.id">
            ...
            <div class="indexLinkBox">
              <flexbox :gutter="0"
                ...
                <flexbox-item :span="1/3"
                              v-for='item in homeData.ideaBanner'
                              @click.native='goLinkFn(item)'
                              :key="item.id">
                  <div class="linkBox">
                    <img :src="item.imgUrl" />
            ...
            <div class="blockContBox"
                 v-if='homeData.recommendArticle'>
              <p class="blockTitle">社区精选</p>
              <div class="flexBox blockBox"
                   v-for='item in homeData.recommendArticle'
                   @click='goLinkFn(item)'
                   :key="item.id">
                <div class="flex1">
                  <p class="articleTitle TextEllipsis">{{item.title}}</p>
                  <p class="articleTip">{{item.typeName}}
        ...
        <div slot="pulldown"
          ...
          <span class="pulldown-arrow"
                v-show="status1.pulldownStatus === 'down' || status1.pulldownStatus === 'up'"
                :class="{'rotate': status1.pulldownStatus === 'up'}">下拉刷新</span>
          <span v-show="status1.pulldownStatus === 'loading'">
    ...
    <div v-transfer-dom>
      <x-dialog v-model="showAdverBlur"
                class="IndexDialogBox"
                hide-on-blur>
        <div class="IndexDialog">
          <img :src="adBanner.imgUrl"
               @click='goLinkFn(adBanner)'>
          <p>
...
export default {
  ...
  methods: {
    weChatLogin: function() {
      let self = this
      self.$http
        .get("h9/api/wechat/login?code=" + self.WxCode)
        .then(function(res) {
          if (res.data.code == 0) {
            localStorage.setItem("_user", JSON.stringify(res.data.data))
            Vue.http.defaults.headers.token = res.data.data.token
              ? res.data.data.token
              : ""
            self.init()
          }
    ...
    init() {
      let self = this
      self.$http.get("h9/api/home").then(res => {
        if (res.data.code == 0) {
