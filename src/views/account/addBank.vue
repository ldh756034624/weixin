<template>
  <div class="page addBank">
    <group class="groupNoTop groupNoLine">
      <x-input title='持卡人' v-model="cardData.name" :show-clear="false" :min="2" :max="10" is-type="china-name"
               placeholder='请输入姓名'></x-input>
      <x-input title='银行卡号' v-model="cardData.no" :show-clear="false" keyboard="number" type='number'
               placeholder='请输入卡号'></x-input>
          ...
          self.$http.post('h9/api/bankCard/add', this.cardData)
            .then(function (res) {
              if (res.data.code == 0) {
                ...
                if (self.$route.query.hasCard == 0 && self.$route.query.type) {
                  self.$router.replace({path: '/account/funds'})
                } else if (self.$route.query.type) {
